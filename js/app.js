// ============================================
// Supabase 客户端初始化 & 工具函数
// ============================================

// 检查配置
if (!SUPABASE_CONFIG.url || SUPABASE_CONFIG.url === 'YOUR_SUPABASE_URL') {
  console.error(
    '%c⚠️ 请先在 js/config.js 中配置你的 Supabase 信息！%c\n' +
    '在 Supabase Dashboard → Settings → API 获取 URL 和 anon key',
    'color: #f59e0b; font-size: 1.1em;', ''
  );
}

// 初始化 Supabase 客户端（使用 CDN 加载的 supabase 全局变量）
const supabase = window.supabase?.createClient
  ? window.supabase.createClient(SUPABASE_CONFIG.url, SUPABASE_CONFIG.anonKey)
  : null;

if (!supabase) {
  console.error('Supabase client 加载失败，请确保页面加载了 supabase-js CDN');
}
...
async function signUp(email, password, username) {
  const { data, error } = await supabase.auth.signUp({
    ...
    options: {
      data: { username },
      emailRedirectTo: `${SUPABASE_CONFIG.siteUrl}/login.html`,
    },
  });
  return { data, error };
}
...
async function resetPassword(email) {
  const { data, error } = await supabase.auth.resetPasswordForEmail(email, {
    redirectTo: `${SUPABASE_CONFIG.siteUrl}/update-password.html`,
  });
  return { data, error };
}
...
async function deleteMedia(url) {
  ...
  if (!pathMatch) return { error: new Error('无法解析文件路径') };

  const { error } = await supabase.storage.from('post-media').remove([pathMatch[1]]);
  return { error };
}
  ...
  return mediaUrls.map(media => {
    ...
    if (media.type === 'image') {
      ...
      if (ytId) {
        ...
        html += `<iframe src="https://player.bilibili.com/player.html?bvid=${blId}" 
    ...
    if (media.caption) {
      html += `<p class="media-caption">${escapeHtml(media.caption)}</p>`;
    }
    html += '</div>';
    return html;
  }).join('');
